            ...
            <form id="informationForm" method="POST" enctype="multipart/form-data">
               <div class="card-body">
                  <div class="rounded-circle avatar avatar-xl mb-3">
                     <label for="uploadBtn" class="mb-0">
                        <img class="rounded-circle avatar-image" id="preview_avatar" src="{{ asset('/path/cdn/avatars/'.Auth::user()->avatar) }}" width="100" height="100">
                     </label>
                  </div>
                  <div class="form-group mb-2">
                     <input id="avatar" type="file" name="avatar" hidden accept="image/png, image/jpeg, image/jpg"/>
                     <button id="uploadBtn" type="button" class="btn btn-secondary mb-2">{{__('Upload Avatar')}}</button>
                     @if(Auth::user()->avatar != 'default.png')
                        <button id="removedBtnAvatar" type="button" class="btn btn-warning mb-2 removeAvatar">{{__('Remove Avatar')}}</button>
                     @endif
                   </div>
