(function($) {
    ...
    $(document).ready(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if ($.isEmptyObject(response.error)) {
                                $('.image-table' + id).remove(); // remove image
        ...
        $("body").on("submit", "#editUserForm", function(e) {
            ...
            $.ajax({
                ...
                type: "post",
                data: {
                    'user_id': $('#user_id').val(), // User id
                    'name': $('#name').val(), // User name
                    'email': $('#email').val(), // User name
                    'status': $('#status').val(), // status
                    'permission': $('#permission').val(), // permission
                    'category': $('#category').val(), // category
                },
                dataType: 'json',
                success: function(data) {
                    $(".editUserBtn").prop("disabled", false); // active add button when on success
                    if ($.isEmptyObject(data.error)) {
                        $(".print-error-msg").css('display', 'none'); // hide errors
    ...
    // Add new preset
    $("body").on("click", "#addPreset", function(e) {
        e.preventDefault();
        ...
        $.ajax({
            url: BASE_URL + '/admin/preset/add',
            type: "post",
            data: {
                'preset': $('#preset').val(), // preset name
                'width': $('#width').val(), // preset width
                'height': $('#height').val(), // preset height
                'content': $('#content').val(), // admin password
            ...
            success: function(data) {
                ...
                if ($.isEmptyObject(data.error)) {
                    $(".print-error-msg").css('display', 'none'); // hide errors
                    $('#modal-simple').modal('hide'); // hide modal
                    $("#addPresetForm")[0].reset(); // reset form
                    swal("Success!", data.success, {
                        icon: "success",
                        buttons: {
                            confirm: {
    ...
    // Edit preset
    $("body").on("click", "#editPresetBtn", function(e) {
        e.preventDefault();
        $(".editPresetBtn").prop("disabled", true); // disable add button when click
        var _active;
        if($("#active").is(":checked")){
            _active = 1;
        } else {
        ...
        $.ajax({
            url: BASE_URL + '/admin/preset/edit/store',
            type: "post",
            data: {
                'preset_id': $('#preset_id').val(), // Page id
                'preset': $('#preset').val(), // preset name
                'width': $('#width').val(), // preset width
                'height': $('#height').val(), // preset height
                'content': $('#content').val(), // admin password
            ...
            dataType: 'json',
            success: function(data) {
                $(".editPresetBtn").prop("disabled", false); // active add button when on success
                if ($.isEmptyObject(data.error)) {
                    $(".print-error-msg").css('display', 'none'); // hide errors
                    $("#editPresetForm")[0].reset(); // reset form
                    swal("Success!", data.success, {
                        icon: "success",
                        buttons: {
                            confirm: {
    ...
    // Delete preset
    $("body").on("click", "#deletePreset", function(e) {
        e.preventDefault();
        swal({
            icon: "info",
            title: 'Are you sure?',
            ...
            if (willDelete) {
                ...
                $.ajax({
                    url: BASE_URL + "/admin/preset/delete/" + id,
                    type: 'delete',
                    dataType: "JSON",
                    data: {
                        "id": id
    ...
    $("body").on("click", "#addCategory", function(e) {
        ...
        $.ajax({
            ...
            type: "post",
            data: {
                'category': $('#category').val(), // preset name
    ...
    $("body").on("click", "#editCategoryBtn", function(e) {
        e.preventDefault();
        $(".editCategoryBtn").prop("disabled", true); // disable add button when click
        var _active;
        if($("#active").is(":checked")){
            _active = 1;
        } else {
        ...
        $.ajax({
            url: BASE_URL + '/admin/category/edit/store',
            type: "post",
            data: {
                'category_id': $('#category_id').val(), // Category id
                'category': $('#category').val(), // Category name
                'active': _active, // active
                'featured': _featured, // active
            },
            dataType: 'json',
            success: function(data) {
                $(".editCategoryBtn").prop("disabled", false); // active add button when on success
                if ($.isEmptyObject(data.error)) {
                    $(".print-error-msg").css('display', 'none'); // hide errors
